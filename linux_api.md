...
**Обработчик выхода** - пользовательская функция, регистрируемая во время выполнения программы и вызываемая при *штатном* завершении процесса функцией `exit()` или возвратом из `main()`.
...
Если какой-либо поток группы выполнит `exec()`, то все **обыкновенные члены завершатся**, а **лидер** начнет выполнять **новую программу**.
...
#### 7.1.2.3. Однократная инициализация
...
Отправляет сигнал самому `sig` вызывающему процессу (а точнее *потоку*). Доставка сигнала происходит *мгновенно* - до завершения данного вызова!
...
Возвращает набор ожидающих сигналов: *стандартных* и *реального времени*.
...
### 8.6.3. Альтернативный стек обработки
...
Посылает сигнал реального времени `sig` в указанный процесс. Если `sig = 0`, то вместо отправки сигнала выполняет проверку прав процесса-отправителя на возможность его доставки получателю. Алгоритм проверки прав совпадает с описанным в вызове `kill()`.
