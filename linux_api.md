...
+ если файловая система смонтирована командой `mount -o nogrpid` (по умолчанию), то группа файла наследуется от действующего GID процесса, если для *родительского каталога* НЕ установлен бит *set-group-ID* - в этом случае группа файла будет наследоваться от него.
...
+ `st_mode` - битовая маска содержащая *тип файла*, установленные для него *биты*, *права доступа* к нему;
