...
Возвращает значение указанного *неизменяемого или динамически ослабляемого ограничения* либо *общесистемной возможности*. Имя, передаваемое в аргумент `name`, должно иметь формат `_SC_XXX`.
...
Возвращает значение указанного *ограничения либо возможности, связанных с файлом (каталогом)*, заданным через путевое имя или дескриптор. Имя, передаваемое в аргумент `name`, должно иметь формат `_PC_XXX`.

# 4. Информация о системе и процессе
...
Для повышения скорости ввода-вывода, осуществляемого системными вызовами или функциями стандартной библиотеки C при работе с дисковыми файлами, применяется *буферизация* данных - это позволяет при каждом чтении/записи не ожидать завершения долгой (по сравнению со скоростью работы памяти и процессора) дисковой операции.
Вызовы `read()` и `write()` не ининциируют непосредственный доступ к диску, а вместо это копируют данные из пользовательского пространства в *буферы дискового кеша ядра*. Ядро НЕ накладывает ограничений на дисковые буферы - оно выделяет столько страниц памяти, сколько требуется процессу. Вызов `write()` возвращается сразу же, после выполнения переноса данных. Вызов `read()` в первую очередь ищет запрошенный диапазон байтов в дисковом кеше и в случае их отсутствия выполняет чтение данных с диска в кеш с запасом - *упреждающее чтение*.

![буферизация файлового ввода-вывода](/images/img-9.png)

### 5.5.1. Буферизация в библиотеке stdio

Функции ввода-вывода стандартной библиотеки С применяют буферизацию в *пользовательском пространстве* памяти для сокращения количества реально выполняемых системных вызовов. Данная буферизация применяется функциями:
...
+ `_IONBF` - НЕ выполнять буферизацию; вызовы функций stdio будут приводить к немедленному выполнению системных вызовов;
+ `_IOLBF` - применять *построчную буферизацию*: накопление символов в потоке осуществляется до появления *символа новой строки* - после этого происходит запись данных либо их завершение их чтения; *по умолчанию* применяется для *терминальных устройств*;
+ `_IOFBF` - использовать *полную буферизацию*: данные будут записываться либо считываться блоками, равныму размеру используемого буфера; *по умолчанию* применяется для *дисковых файлов*.

Аргумент `buf` может содержать либо заранее выделенный вручную буфер либо `NULL` - в этом случае библиотека самостоятельно выделит буфер некоторого оптимального размера.
...
Значения пропущенных аругментов заполняются библиотекой самостоятельно.
...
+ **завершение с целостностью данных** - при обновлении файла должен быть обеспечен перенос информации, достаточной для последующего успешного извлечения его данных:
  + *для операций записи* - должно быть перенесено содержимое файла, а также *необходимые метаданные* (например, результирующий размер файла, но не обязательно метки времени);
  + *для операций чтения* - должно быть перенесено содержимое файла с предварительным завершением всех затрагивающих его отложенных операций записи;
...
Ядро предоставляет процессу возможность проинформировать себя о предполагаемом характере обращения к содержимому указанного файла, что позволит в данному конкретном случае оптимизовать работу механизма дисковых буферов. Однако не даётся никаких гарантий, что ядро воспользуется предоставленными ему сведениями.
...
По своей природе устройство может являться *реальным* либо *виртуальным* - в этом случае ядро предоставляет абстракцию с набором поддерживаемых операций, аналогичным реальному устройству.

Устройство должно предоставлять системе требуемый ею набор операций в заданном формате - т.е. должно реализовывать определенный интерфейс, которым зачастую является универсальный набор операций ввода-вывода, позволяющий добиться единообразия коммуникации с устройствами различных типов. Реализацию требуемого интерфейса осуществляет **драйвер устройства** - модуль ядра (обычно произведенный третьей стороной), сопрягающий систему и реально имеющийся функционал устройства.

Связь устройства и его драйвера устанавливается с помощью **файла устройства** - экземпляра специальной сущности файловой системы, содержащегося в **каталоге `/dev`** по одному для каждого подключенного к системе устройства.
*Индексный дескриптор* файла устройства содержит:

+ **старший идентификационный номер устройства (major)** - определяет *класс устройства*; администрированием данных идентификаторов занимается организация *Linux Assigned Names and Numbers Authority (LANANA)*;
+ **младший идентификационный номер устройства (minor)** - уникальным образом определяет устройство внутри его класса.

При установке *драйвер* регистрирует свою *привязку* к конкретному *старшему идентификатору* и таким образом информирует ядро о видах устройств, за обработку которых он отвечает. Поскольку связь устройства и его драйвера осуществляется на основе старшего идентификатора, то имя файла устройства в данном случае значения не имеет.

Различная сервисная информация об устройствах системы предоставляется **виртуальной файловой системой `/sys`**.
...
+ **область подкачки** - копии не используемыех в настоящих момент физических страниц, которые не уместились в имеющемся объеме оперативной памяти. За создание файлов подкачки отвечают утилиты `mkswap`, а также `swapon/swapoff` (существует аналогичный системный вызов, требующий привилегии процесса `CAP_SYS_ADMIN`). Файл `/proc/swaps` перечисляет все имеющиеся в системе файлы подкачки.

## 6.3. Файловые системы

**Файловая система** - упорядоченный набор файлов и каталогов. Linux поддерживает самые разные реализации файловых систем, обладающих своими собственными характерными особенностями:

+ сетевые: NFS;
+ журналируемые: XFS, ext3, ext4;
+ родные для Linux: ext2, ext3, ext4;
+ за авторством Microsoft: FAT, FAT32, NTFS.

Наименьшей единицей выделения пространства в файловой системе является *логический блок*, представляющий собой совокупность смежных *физических блоков* заданного размера. Типичная файловая система образована следующими составными частями:

+ **блок начальной загрузки** - *всегда самый первый блок* в файловой системе, содержащий информацию о загрузке файловой системы; *каждая файловая система*, располагающаяся в своем разделе диска, обладает собственным блоком начальной загрузки;
...
**Индексный дескриптор (i-node)** - стуктура данных, содержащая метаинформацию о взаимно однозначно связанном с ней файле или каталоге. Все индексные дескрипторы уникальным образом идентифицируются по своему порядковому номеру в таблице. Индесексный дескриптор содержит следующие поля:

+ **тип файла** - является ли сущность файловой системы обычным файлом, каталогом, *символической ссылкой* и т.д.;
...
**указатели на блоки данных** - разделяются на 2 типа:
  + *прямые* - непосредственно указывают на блок, содержащий данные файла;
  + *косвенные* - указывают на блок, содержащий список указателей (от 256 до 1024 штук); косвенность может быть *двойной* либо *тройной*.
