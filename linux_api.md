...
**Индексный дескриптор (i-node)** - структура данных, содержащая метаинформацию о связанном с ней файле или каталоге. Все индексные дескрипторы однозначным образом идентифицируются по своему порядковому номеру в данной таблице. Индексный дескриптор содержит следующие поля:

+ **тип файла** - определяет, является сущность файловой системы обычным файлом, каталогом, *символической ссылкой* и т.д.;
...
+ `/etc/mtab` - аналог предыдущего файла, заполняемый командами оболочки `mount`, `umount` и содержащий более детализированную информацию;
...
Монтирует файловую систему устройства, файл которого имеет путевое имя `source`, в каталог (*точку монтирования*) `target`.

Аргумент `fstype` содержит строку, определяющую тип файловой системы: ext4, btrfs и т.д.

Аргумент `mountflags` содержит битовую маску, управляющую ходом выполнения данного вызова, а аргумент `data` - дополнительные параметры вызова:
