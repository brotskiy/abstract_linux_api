...
## 7.4. Установка диспозиции сигнала
...
Данный вызов **НЕ ПЕРЕНОСИМ** - вместо него предпочтительно использование API `sigaction()`.

Аргумент `handler` также может следующие константы:

+ `SIG_DFL` - сбросить диспозицию на поведение по умолчанию;
