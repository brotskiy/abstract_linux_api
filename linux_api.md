...
Поиск библиотечных файлов выполняется в определенном наборе каталогов в соответствии со следующим приоритетом:

1. каталоги, перечисленные в переменной окружения `LD_LIBRARY_PATH`; **НЕ рекомендуется использовать!**
2. стандартные библиотечные каталоги системы:
   1. `/lib` - содержит библиотеки, необходимые во время загрузки системы;
   2. `/usr/lib` - содержит большинство прикладных библиотек;
   3. `/usr/local/lib` - содержит нестандартные или экспериментальные библиотеки;
   4. один из каталогов, перечисленных в файле `/etc/ls.so.conf`
...
### 8.2.2. Версионирование разделяемых библиотек
...
### 8.2.3. Команды оболочки для работы с разделяемыми библиотеками
