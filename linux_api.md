...
По этой причине ядро Linux *аварийно завершает* процессы, игнорирующие или блокирующие доставку *аппаратных сигналов*. Единственным правильным способом их обработки является выполнение нелокального перехода из обработчика `siglongjump()`.
...
+ *по завершении системного вызова* (блокирующие вызовы могут преждевременно завершаться с кодом `EINTR`);
...
### 7.5.3. Приостановка процесса в ожидании сигнала
...
+ `sa_flags` - маска флагов, управляющих поведением вызова:
  ...
  + `SA_RESTART` - **автоматически перезапустить** блокирующий системный вызов (с неопределенным временем завершения: ввод/вывод, синхронизация и проч.), который оказался прерван с кодом возврата `EINTR` доставкой сигнала в процесс.
...
Иногда нужно завершить *обработчик сигнала*, вернувшись в конкретный участок программы, и при этом восстановить исходное значение *сигнальной маски* процесса, которая могла быть модифицирована обработчиком. Например, это может потребоваться, чтобы восстановить работоспособность программы после получения сигнала, сгенерированного аппаратным исключением.
...
Единственный способ продолжить выполнение программы после вызова `abort()` - это определить для данного сигнала обработчик, выполняющий *нелокальный переход* `siglongjmp()`.
...
## 7.7. Отображение описаний сигналов
