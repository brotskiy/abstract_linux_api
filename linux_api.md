...
Альтернатива вызову `fork()`, особенности реализации которой позволяют эффективно порождать дочерние процессы, которые сразу же должны сделать вызов `exec()`, загружающий новую программу:

+ дочерний и родительский процессы **разделяют общую виртуальную память**, пока кто-то из них не вызовет `exec()` или `_exit()`;
+ родитель приостанавливается, пока потомок не вызовет `exec()` или `_exit()`;

Данный вызов был нужен в раннем Unix, когда не применялись оптимизации памяти, и позволял потомку, сразу же вызывающему `exec()`, не выполнять ненужное копирование памяти родителя.
