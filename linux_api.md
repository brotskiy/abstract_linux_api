...
3. затем выполняется последовательное сканирование всех **динамически загруженных** библиотек (вместе с их деревьями зависимостей) в порядке их загрузки вызовом `dlopen(RTLD_GLOBAL)`, и ссылка привязывается к первому обнаруженному определению.
...
+ `RTLD_GLOBAL` - символы из текущей библиотеки и её дерева зависимостей можно использовать для разрешения ссылок в загружаемых впоследствии библиотеках;
...
+ `RTLD_DEEPBIND` - при разрешении ссылок текущей библиотеки отдавать предпочтение её собственным глобальным символам перед символами из библиотек, загруженных ранее. Является аналогом параметра `-Wl,-Bsymbolic` *динамической компоновки*;
