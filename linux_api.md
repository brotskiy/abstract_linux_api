...
> При **статической компоновке** с программой модуля **статического или динамического модуля** его имя может быть указано одним из следующих способов:
>
> + **полное имя** файла библиотеки: `libname.a`;
...
>
> + **название библиотеки с префиксом `-l`** и **путь до файла с префиксом `-L`**: `-lname -L/path/to/lib`.
...
Затем статический компоновщик должен встроить **имя библиотеки** в метку `DT_NEEDED` ELF-заголовка исполняемого файла программы - это также выполняется **статическим компоновщиком** `ld`, неявно вызываемым компилятором при сборке программы.
...
+ `p_flags` - флаги доступа к страницам памяти, в который оказался загружен сегмент:
...
2. **Position-independent code (PIC)** - адресно-независимый код.
    ...
    + текстовый сегмент теперь не требует разрешения на запись, что согласуется с требованиями безопасности;
    ...
    1. когда функция DSO впервые вызывается теле библиотеки или программы начинается выполнение кода её записи в таблице *PLT*;
    ...
    Необходимость вызывать внешние функции библиотеки косвенно через команды ячеек PLT и запускать поиск их определений резолвером является причиной ЗАПРЕТА на создание в коде библиотек или программ *непосредственных указателей на внешне компнуемые функции*. Чтобы на библиотечную функцию можно было создавать и использовать указатель, она должна быть обозначена как адресуемая (**addressable function**).
...
Правила поиска определения символа в областях при выполнении его *перемещения (relocation)*:
...
На примере выше сначала собирается библиотечный модуль с заданным реальным именем и псевдонимом soname. Псевдоним soname окажется встроен в тело программый и будет искаться динамическим компоновщиком при её запуске. То, какая именно версия библиотеки будет загружаться в память, можно регулировать установкой *символической ссылки soname* на разные целевые файлы. Затем выполняется сборка программы - в этот момент псевдоним soname библиотеки встраивается в тело программы. То, какая именно версия библиотеки будет служить источником soname, можно регулировать установкой *символической ссылки компоновочного имени* на разные целевые файлы.
