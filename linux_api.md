...
+ `sa_flags` - маска флагов, управляющих поведением вызова:
  ...
  + `SA_SIGINFO` - использовать для функции-обработчика **расширенную сигнатуру** `sa_sigaction`, позволяющую передавать вместе с сигналом сопутствующие данные.
    ...
    + `si_timerid`, `si_overrun` - описание таймера;
...
**Сигнальный интервальный таймер** действует на всём протяжении `exec()`, но **не наследуется в дочерние процессы** в результате `fork()`.

## 10.2. Интервальные таймеры POSIX

Для использования **интервальных таймеров POSIX** пользовательская программа должна быть скомпилирована с ключом `-lrt` для возможности компоновки с *библиотекой реального времени* `librt`.

*Интервальные таймеры POSIX* в течение `exec()` могут создаваться и удаляться, но **не наследуется в дочерние процессы** в результате `fork()`.
...
Для *каждого* создаваемого в вызывающем процессе POSIX-таймера ядро заранее выделяет структуру для хранения *сигнала реального времени*. Поэтому количество *POSIX-таймеров* процесса ограничено количеством доступных ему *сигналов реального времени*!
