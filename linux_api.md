...
**Разделяемая (динамическая) библиотека** - бинарный файл, объединяющий в себе *как единое целое* *адресно-независимый* код объектных файлов, из которых он состоит. Используемые программой разделяемые модули загружаются в её память **динамическим компоновщиком** `/lib/ld-linux.so` только во время выполнения. Несколько программ, использующих одну и ту же разделяемую библиотеку, отображают свои страницы виртуальной памяти на одни и те же физические страницы, содержащие её код в единственном экземпляре - это способствует экономному использованию оперативной памяти.
...
Поиск файла разделяемой библиотеки **динамическим компоновщиком** при запуске программы выполняется в следующем порядке:
...
Если *глобальный символ* (функция или переменная) **определен** сразу в нескольких местах, то для разрешения ссылки на него используются следующие правила:

+ определение глобального символа в главной программе перекрывает определение в библиотеке;
+ если глобальный символ определен сразу в нескольких библиотеках, то ссылка на него привязывается к первому определению, найденному в ходе последовательного сканирования библиотек в порядке, в котором они были указаны в команде **статической компоновки**;
+ если глобальный символ НЕ был найден в **динамически скомпонованных** библиотеках, то его поиск продолжается в **динамически загруженных** библиотеках в порядке вызовов `dlopen(RTLD_GLOBAL)` (см. далее).

Следование этим правилам приводит к тому, что по умолчанию библиотека при своём включении в программу наряду с другими библиотеками НЕ может гарантировать, что её внутренние ссылки в итоге будут привязаны исключительно к её собственным определениям.

Параметр компоновки `-Wl,-Bsymbolic` позволяет библиотеке обеспечивать эту гарантию: ссылки на глобальные символы внутри самой библиотеки в первую очередь будут привязываться к определениям из этой библиотеке (если таковые существуют).
...
### 8.2.3. Совместное использование статических и разделяемых библиотек
...
### 8.2.4. Версионирование разделяемых библиотек
...
### 8.2.5. Команды оболочки для работы с разделяемыми библиотеками
...
### 8.2.6. Динамическая загрузка библиотек. Интерфес dlopen()
