...
+ `CLONE_THREAD` - потомок будет создан в **группе потоков** родителя, иначе он будет помещен в свою отдельную группу. Требует наличия флага `CLONE_SIGHAND`.
    ...
    Каждый поток имеет уникальный в рамках все системы **Thread Identifier (TID)**. Все члены одной **группы потоков** имеют одинаковый **Thread Group Identifier (TGID)**, и фактически **TGID = PID**. Первый поток группы является **лидирующим**, и для него **TID = TGID**. Родителем всех потоков в группе является *родитель лидера*.

    ![группа потоков](images/img-27.png)

    *Родитель группы* получает `SIGCHLD`, когда *все* потоки группы завершают выполнение. *Лидер группы* не получает сигналов при завершении членов и может ожидать их с помощтю `wait()` - потоки нужно *присоединять* через `pthread_join()`.
...
## 7.5. Блокировка и ожидание сигналов

### 7.5.1. Заблокированные и ожидающие сигналы
...
### 7.5.2. Приостановка процесса в ожидании сигнала
...
### 7.6.1. Установка диспозиций сигналов
...
### 7.6.2. Нелокальный переход из обработчика
...
### 7.6.3. Альтернативный стек обработки
...
## 7.8. Отображение описаний сигналов
...
### 8.1.1. Создание статических библиотек
...
### 8.2.1. Создание разделяемых библиотек
...
### 8.2.2. Разрешение символов на этапе выполнения
...
#### 8.2.2.3. Порядок поиска определений символов
...
### 8.2.3. Динамическая загрузка библиотек dlopen
...
### 8.2.4. Правила именования разделяемых библиотек
...
### 8.2.5. Совместное использование статических и разделяемых библиотек
...
### 8.2.6. Команды оболочки для работы с разделяемыми библиотеками
...
## 9.1. Структуры данных для хранения времени
...
## 10.1. Сигнальные интервальные таймеры (УСТАРЕЛИ)
...
## 10.2. Сигнальные интервальные таймеры POSIX
...
## 10.3. Интервальные таймеры timerfd
