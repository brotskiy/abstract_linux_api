...
Количество тиков в секунде определяется значением `sysconf(_SC_CLK_TCK)`. Значение количества тиков типа `clock_t` не застраховано от переполнения - в этом случае отсчет начнется с 0!
