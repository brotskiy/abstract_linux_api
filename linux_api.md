...
3. Затем если задана переменная окружения `LD_LIBRARY_PATH`, то поиск зависимости выполняется в её каталогах в порядке их следования. При этом если запускаемая программа устанавливает биты *set-user-ID* или *set-group-ID*, то данная переменная **игнорируется в целях безопасности**;
...
5. Затем поиск зависимости выполняется в каталоге `/lib` и после в `/usr/lib`.
