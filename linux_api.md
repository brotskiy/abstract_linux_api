...
2. **Position-independent code (PIC)** - адресно-независимый код.
   ...
   + однако косвенный переход через промежуточную структуру требует большего количества инструкций, чем прямое обращение к символу - это является издержкой использования адресно-независимого кода.
    ...
    Для обращения к данным DSO использует **Global Offset Table (GOT)** - *глобальную таблицу смещений*. Для неё в сегменте данных DSO создаётся логически обособленная секция под названием `got`, записи которой хранят адреса переменных. Кроме того, в ELF-секцию `rel.dyn` помещается *таблица динамических перемещений*, описывающая корректировки всех адресов таблицы смещений - в процессе загрузки библиотеки поместить фактический адрес переменной в соответствующую ему запись GOT.
