...
Механизм разрешения **динамическим компоновщиком** ссылок на символы намеренно спроектирован таким образом, чтобы выполнять поиск определений *во всем множестве* загруженных в память процесса бинарных объектов, а не только в том объекте, который содержит ссылку на символ. Данная логика обеспечивается существованием *глобальной области поиска* (**global lookup scope**), представляющей собой *упорядоченный список* загруженных объектов:
...
Порядок поиска **определения** *глобального символа* (функции или глобальной либо статической переменной) при резрешении ссылки на него также строго задан:

1. в первую очередь глобальный символ ищется в главной программе;
2. затем выполняется последовательное сканирование всех **динамически скомпонованных** библиотек (вместе с их деревьями зависимостей) в порядке их указания в команде **статической компоновки**, и ссылка привязывается к первому обнаруженному определению;
3. затем выполняется последовательное сканирование **динамически загруженных** библиотек (вместе с их деревьями зависимостей) в порядке их загрузки вызовом `dlopen(RTLD_GLOBAL)`, и ссылка привязывается к первому обнаруженному определению.

    Поиск, выполняемый тремя описанными шагами, производился в **глобальном пространстве символов**.

4. затем, если поиск инициирован вызовом `dlsym()` для библиотеки, **динамически загруженной** вызовом `dlopen(RTLD_LOCAL)` (вместе с её деревом зависимостей), определение ищется в **локальном пространстве символов** данной библиотеки.

Описанный порядок поиска символов приводит к тому, что по умолчанию библиотека при своём включении в программу наряду с другими зависимостями НЕ может гарантировать, что её внутренние ссылки будут привязаны исключительно к её собственным определениям. Параметр компоновки `-Wl,-Bsymbolic` позволяет библиотеке обеспечивать эту гарантию: ссылки на глобальные символы внутри самой библиотеки в первую очередь будут привязываться к определениям из этой библиотеке (если таковые существуют).
