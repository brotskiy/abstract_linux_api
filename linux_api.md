...
+ **UID и GID файловой системы** - идентификаторы, используемые при выполнении процессом *операций в файловой системе*. Существуют по историческим причинам и практически всегда *совпадают с действующими идентификаторами*;
+ **дополнительные ID групп** - идентификаторы групп, в которых состоит процесс. Процесс *наследует* данные идентификаторы от своего *родителя*. Процесс *оболочки входа* пользователя, являющийся родителем всех процессов прикладных программ, получает их из *файла групп* `/etc/group`.
...
Возвращают *действующи* идентификатор.
...
Если нужно изменить только один идентификатор, то второму присваивается значение `-1`.
