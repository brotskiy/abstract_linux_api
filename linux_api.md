...
### 6.3.2. Выполнение новой программы: `exec()`
...
#### 6.3.2.1. Файловые дескрипторы и `exec()`
...
#### 6.3.2.3. Выполнение интерпретируемых скриптов через `exec()`
...
### 6.3.3. Ожидание дочернего процесса: `wait()`
...
Аналоги вызова `waitpid()`, дополнительно возвращащие в аргументе `rusage` информацию об использовании потомком ресурсов системы: процессорного времени и памяти. Не переносимы.
...
#### 6.3.3.1. Статус завершения потомка
...
### 6.3.4. Завершение процесса: `exit()`
...
```C
typedef void (*func)(exit_func_t, void*);
