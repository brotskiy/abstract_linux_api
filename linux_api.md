...
1. **Load-time relocation** - перемещение времени загрузки.
   ...
   + модификация текстового сегмента DSO означает, что различные процессы не могут отображать свои виртуальные страницы на общий для всех набор физических страниц оперативной памяти с кодом DSO. Это нивелирует существенное преимущество разделяемых библиотек перед статическими.
...
Когда библиотека динамически загружается вызовом `dlopen()`, то для неё создаётся отдельная *локальная область поиска* (**local lookup scope**). В эту область поиска также помещаются все её зависимости, **загружаемые впервые**: т.е. в настоящий момент *отсутствующие в глобальной области* (но, возможно, *присутствующие в *других локальных областях*). Локальная область по своей структуре и способу заполнения повторяет глобальную область (упорядоченный список, рекурсивный обход в ширину). Параметр `RTLD_GLOBAL` добавляет созданную *локальную область* в конец *глобальной области*, а `RTLD_LOCAL` этого не делает.
