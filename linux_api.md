...
Если аргумент `flags = 0`, то `request` задаёт *длительность* сна. Если `flags = TIMER_ABSTIME`, то `request` задаёт *абсолютную временную метку* в будущем, по достижении которой процесс должен проснуться. *Абсолютное значение* времени не подвержено накоплению избыточной длительности сна при перезапусках вызова в случае поступления сигналов с высокой частотой: т.к. *оставшаяся длительность* сна при перезапуска зачастую не кратна точности часов, то к каждый повторный сон будет содержать лишнюю прибавку, образованную округлением до точности в большую сторону.
...
+ `CLOCK_REALTIME` - общесистемные часы *реального календарного времени*, допускающие изменение;
+ `CLOCK_MONOTONIC` - *монотонно возрастающие* часы, измеряющие время с некоторого заданного момента в прошлом (для Linux со старта системы), не подверженные и не допускающие ручное изменение времени;
...
+ `CLOCK_MONOTONIC_RAW` - часы с сырым аппаратным временем;
...
Значение таймаута `new_value.it_value` уменьшается системой до нуля - в этот момент происходит срабатывание таймера. Если значение интервала `new_value.it_interval` не равнялось нулю, то оно переносится в таймаут, и обратный отсчет запускается заново - таймер является периодическим.
