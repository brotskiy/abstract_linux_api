...
2. **Position-independent code (PIC)** - адресно-независимый код.
    ...
    Адреса *функций с внешней компоновкой* также хранятся в таблице **GOT**, расположенной в секции `got.plt` (соседствует с таблицей адресов переменных). Однако их число, как правило, значительно больше числа переменных в области данных библиотеки. Кроме того, не все определённые в библиотеке функции обычно используются программой. Поэтому временные затраты на поиск всех определений и корректировку их адресов в момент старта библиотеки кажутся избыточными, и возникает желание их избежать. Для этих целей DSO применяет механизм **lazy binding** - поиск определения функции выполняется только в момент её первого вызова. Для его реализации в логически обособленную секцию *текстового сегмента* DSO под названием `plt` помещается **Procedure Linkage Table (PLT)** - *таблица компоновки процедур*. Каждая её запись содержит последовательность команд запуска соответствующей фунции. Вызов функции DSO в теле библиотеки или программы приводит к выполнению кода её ячейки PLT, который и осуществляет реальный вызов. PLT имеет следующее содержимое:
