...
## 10.1. Интервальные сигнальные таймеры (УСТАРЕЛИ)
...
**Интервальный сигнальный таймер** действует на всём протяжении `exec()`, но **не наследуется в дочерние процессы** в результате `fork()`.
