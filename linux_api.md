...
Запущенный **сигнальный интервальный таймер** живет в пределах `exec()` и **НЕ наследуется в дочерние процессы** в результате `fork()`.
...
Запущенный **сигнальный интервальный таймер POSIX** живет в пределах `exec()` и **НЕ наследуется в дочерние процессы** в результате `fork()`.
...
Запущенный **интервальный таймер timerfd** переживает завершение `exec()` и **наследуется в дочерние процессы** в результате `fork()`. Поэтому по завершении работы с таймером его дескриптор необходимо закрыть через `close()` для освобождения ресурсов.
