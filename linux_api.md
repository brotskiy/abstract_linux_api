...
**Локаль** - совокупность переменных среды, зависящих от языковых и культурных норм. Информация о локали размещается в каталоге `/usr/share/locale` (либо `/usr/lib/locale`), содержащем набор каталогов, каждый из которых описывает соответствующую географическую территорию. Каталоги локалей имеют название следующего вида:
