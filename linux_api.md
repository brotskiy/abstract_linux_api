...
Если файл не существует, то он может быть автоматически создан при открытии (это определяется битовой маской флагов). В этом случае аргумент `mode` применяется для установки *прав доступа* к файлу со стороны *владельца*, *группы* и *остальных* (возможные значения описываются позднее) - в противном случае он может быть опущен. Фактически установленный набор прав определяется комбинацией значений аргумента mode и маски процесса **umask** (либо **default ACL** - списка контроля доступа по умолчанию родительского каталога).
...
+ имеют особую структуру содержимого: содержат таблицу, записи которой представляют собой пары из *имени файла* и *номера* соответствующего *индексного дескриптора*;
...
Имя файла не содержится среди атрибутов индексного дескриптора - оно определяется соответствующей записью внутри каталога. Более того, допускается создание множества записей, связывающих различные имена с единственным индексным дескриптором и соответствующим ему файлом. Такие имена называются **жесткими ссылками**.
...
*Нельзя* создать *жесткую ссылку на каталог*. Причина - предотвращение создания циклических ссылок. Однако можно создать для каталога *связанную точку монтирования*.
...
*Процесс* может отправить сигнал *другому процессу* (а также *самому себе*) в *любой момент*, который он сочтет нужным.

*Ядро* отправляет сигнал процессу в следующих случаях:

+ произошло *аппаратное исключение* - оборудование зафиксировало выполнение недопустимой операции, уведомило ядро, а оно, в свою очередь, оповестило процесс; например, деление на 0, обращение к недопустимому участку памяти, передача ошибочной инструкции и т.д.;
+ произошло определённое *программное событие*; например, завершение дочернего процесса, срабатывание таймера, появление данных для чтения в файловом дескрипторе и т.д.;
+ пользователь *в терминале* ввел специальный символ, генерирующий сигнал: например, *прерывание* `Ctrl+C`, *выход* `Ctrl+\` и т.д.

Все сигналы подразделяются на 2 группы:

+ **Стандартные** - стандарт четко перечисляет их список и назначение. Используются ядром для оповещения процесса и управления его периодом жизни. Сами процессы также могут отправлять стандартные сигналы. Каждому сигналу из этой группы присвоен *фиксированный номер* в пределах `1 - 31`;
+ **Реального времени** - пользователь сам определяет их назначение, условия генерации и нумерацию.
...
*По умолчанию* для каждого сигнала четко задано, причиной какого действия в процессе он является. Возможны следующие **действия по умолчанию**:

+ *Игнорирование сигнала* - сигнал ОТБРАСЫВАЕТСЯ ядром без уведомления процесса;
+ *Мгновенное завершение* процесса - выполнение процесса прерывается в обход логики выполняемой им программы;
+ *Мгновенное завершение* с созданием **файла дампа ядра** - будет создан файл, содержащий отпечаток *виртуальной памяти* процесса (он может быть загружен в *отладчик* для последующего анализа);
+ *Приостановка* процесса;
+ *Возобновление* процесса.

Для каждого сигнала процесс может определить действие, выполняемое при его получении - выполнить **установку диспозиции сигнала**:

+ Выполнять одно из описанных *действий по умолчанию*;
+ *Игнорировать сигнал*;
+ *Перехватывать* сигнал и выполнять пользовательский *обработчик сигнала*.

Механизмы передачи в процесс стандартных и сигналов реального времени отличаются и подробно будут описаны далее.
...
| `SIGTSTP`            | приостанавливает        | используется в оболочке, поддерживающей *управление заданиями*; приостанавливает *активную группы процессов* вводом *символа приостановки* `Ctrl+Z` |
| `SIGTTIN`            | приостанавливает        | используется в оболочке, поддерживающей *управление заданиями*; приостанавливает *фоновую группы процессов* при её попытке *чтения* с терминала     |
| `SIGTTOU`            | приостанавливает        | используется в оболочке, поддерживающей *управление заданиями*; приостанавливает *фоновую группы процессов* при её попытке *записи* в терминал      |
