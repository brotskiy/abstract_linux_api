...
При обращении процесса к сущности файловой системы по её путевому имени проверки прав доступа выполняются в следующей очередности с остановкой в момент нахождения *первого соответствия*:
...
**Sticky** бит (также называемый *saved text*) в настоящее время устанавливается *только для каталогов* и ограничивает изменение их содержимого: непривилегированный процесс может *удалять и переименовывать* файлы и подкаталоги, только если он является *владельцем файла, подкаталога или всего родительского каталога* (очевидно, что при этом процесс должен обладать *правом на запись* для родительского каталога).
