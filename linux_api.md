...
1. `fork()` - создаёт копию вызывающего *процесса-родителя* - *дочерний* процесс;
2. `exec()` - загружает в память *(дочернего)* процесса новую *программу* и начинает её выполнение;
3. `wait()` - приостанавливает *родительский* процесс, пока не будет завершен один из его *детей*, и возвращает *код* его *завершения*.
...
Раздваивает *родительский* процесс, порождая *дочерний*, который продолжает своё выполнение с места вызова. В *родительский* процесс позвращается **PID потомка**, в *дочерний* процесс возвращается 0.
